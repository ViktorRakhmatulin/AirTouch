...
def arduino_theta_control(coord_variable):
    ...
    while True:
        ...
        if coord_variable.poll():
            ...
            current_distance = np.linalg.norm(marker - x_end)
            speed_dist = (current_distance - previous_distance) / (current_time - previous_time)
            if speed_dist < 0:
                activation_range = 0.2 + 0.1 * speed_dist
                if current_distance < activation_range:
                    pass
