...
class ArduCon():
    ...
    def select(self):
        print(self.Ports)
        print(self.Speeds)
        print("Choose port (by index):")
        chosen_port = input()
        self.chosen_port = self.Ports[int(chosen_port)]
        print(f'Chosen port: {self.chosen_port}')
        print("Choose baud rate (enter value):")
        chosen_speed = input()
        self.chosen_speed = int(chosen_speed)

    def connect(self):
        try:
            self.realport = serial.Serial(self.chosen_port,self.chosen_speed)
...
#        print("[INFO] {} total AprilTags detected".format(len(results)))
        ...
        for r in results:
            ...
            rvec = r.pose_R
            april_distance = numpy.linalg.norm(r.pose_t)
            ...
            cv2.putText(opencvImage,f'Distance: {april_distance:.2f}',(ptA[0]+40, ptA[1] - 65),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
